...
def tprint(txt):
    global ttmsg
    if ttmsg != None :
        ttmsg.insert(tk.END, txt)
        ttmsg.see(tk.END)

#用户输出信息,带颜色
def tcprint(txt,color):
    global ttmsg
    if ttmsg != None :
       ttmsg.tag_config(color, foreground=htk.Tkcolor(color))   
       ttmsg.insert(tk.END, txt,color)
       ttmsg.see(tk.END)
...
class useredit(Frame):# 继承Frame类  
    ...
    def Init(self):
        ...
        self.toolbarCommand = (self.newfile,self.openfile,self.savefile,self.saveas,\
                               self.undo,self.redo,self.cut,self.copy,self.paste,self.selectall,self.runuc0)
       ...
       # 创建弹出菜单
        ...
        self.toolbarCommand2 = (self.newfile,self.openfile,self.savefile,self.saveas,\
                                self.undo,self.redo,self.cut,self.copy,self.paste,self.selectall,self.runuc0)
...
class ClosableNotebook(ttk.Notebook):
    ...
    def _check_update_style(self):
        ...
        if "img_close" not in self.image_names():
            ClosableNotebook._close_img = tk.PhotoImage("img_tab_close",data=(("R0lGODdhDgAKAIcAAMbGxrG1sbGksZ2knQ"+"A"*1007+"CwAAAAADgAKAAAIRwABCBxIsKDBgwMDAFgIIACAhxAhCgAAQACAixgzAhAQAIDHjyA9BhgAoKTJkwAEAAAQAIDLly8DAJgJQACAmzhz6tzJU2dAADs=").encode()))
            ClosableNotebook._close_active_img = tk.PhotoImage("img_tab_close_active",data=(("R0lGODdhDgAKAIcAAMbGxmBgY"+"A"*1016+"CwAAAAADgAKAAAIRwABCBxIsKDBgwMDAFgIIACAhxAhBgAAIACAixgzAggQAIDHjyA9BggAoKTJkwACAAAQAIDLly8DAJgJIACAmzhz6tzJU2dAADs=").encode()))
            ClosableNotebook._tab_icon = tk.PhotoImage("icon",data=b"")
